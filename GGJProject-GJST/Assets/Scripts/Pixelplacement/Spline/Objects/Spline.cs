...
namespace Pixelplacement
{
    ...
    public class Spline : MonoBehaviour
    {
        ...
        private float Reparam(float percent)
        {
            ...
            for (var i = 0; i < _splineReparams.Count; i++)
            {
                var currentPercentage = _splineReparams[i].length / Length;
                ...
                if (currentPercentage > percent)
                {
                    var fromP = _splineReparams[i - 1].length / Length;
                    var toP = currentPercentage;

                    //slide scale to 0:
                    var maxAdjusted = toP - fromP;
                    var percentAdjusted = percent - fromP;

                    //find out percentage:
                    var inBetweenPercentage = percentAdjusted / maxAdjusted;
                    var location = Mathf.Lerp(_splineReparams[i - 1].percentage, _splineReparams[i].percentage, inBetweenPercentage);
        ...
        {
            //prep:
            var totalSlices = (Anchors.Length - 1) * _slicesPerCurve;
            Length = 0;
            ...
            {
                //percent ends:
                var percent = i / (float)totalSlices;
                var previousPercent = (i - 1) / (float)totalSlices;
